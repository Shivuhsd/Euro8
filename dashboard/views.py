...
def SchoolContractView(request, pk):
    get_schedules,html_calendar,year, month, context = "", "", "", "", ""
    today = date.today()
    ...
    try:
        if request.method == 'GET':
            sch = request.GET['schedule']
            sch = Schedule.objects.get(id = sch)
            get_schedules = TrackSchoolContract.objects.filter(contract = sch)
        

            now = datetime.now()
            year = int(now.year)
            month = int(now.month)
            cal = EventCalendar(get_schedules)
            html_calendar = cal.formatmonth(year, month)
    except:
        pass
    context = {
